...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const fetchUserProfile = async () => {
    try {
      const { data, error } = await supabase.from("auth.users").select("public_username").eq("id", user?.id).single()
              ...
              <Button variant="ghost" className="w-full justify-start px-3 py-2 h-auto hover:bg-primary/5">
                <Avatar className="mr-3 h-8 w-8">
                  <AvatarFallback className="bg-primary/10 text-primary text-sm">
                    {publicUsername ? publicUsername.slice(-2).toUpperCase() : user?.email?.charAt(0).toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div className="flex flex-col items-start">
                  <span className="text-sm font-medium truncate max-w-[140px]">{publicUsername || "Loading..."}</span>
